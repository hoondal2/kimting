package com.kimting.kimting.shared.mcp;
...
public class MemoryMcpTool {
    ...
    public String searchMemory(String query) {
        List<Memory> results = memoryUseCase.search(query, 5, null);
        if (results.isEmpty()) {
    ...
    public String recentMemories() {
        List<Memory> results = memoryUseCase.recent(10, null);
        if (results.isEmpty()) {
            return "저장된 기억이 없다.";
        }
        return results.stream()
    ...
    public String memoriesByPerson(String personName) {
        List<Memory> results = memoryUseCase.findByPerson(personName, null);
        if (results.isEmpty()) {
